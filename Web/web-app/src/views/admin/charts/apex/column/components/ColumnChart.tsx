        ...
        events: {
          dataPointSelection: (_e, chart, opts) => {
            ...
            if (selected && selected.length > 0) {
              updateQuarterChart(chart.w.config.series[0] as SourceSeries, selected)
            } else {
              setQuarterData([])
            }
          },
    ...
    <Card>
      ...
      <CardBody className="pt-0">
        ...
        <Row>
          ...
          <Col sm={6}>{quarterOptions && <ReactApexChart options={{ ...quarterOptions, colors }} series={quarterData || []} type="bar" height={400} />}</Col>
        </Row>
      </CardBody>
    </Card>
  )
