    ...
    if (currentView === 'map') {
      return {
        tooltip: {
          ...
          formatter: (params: any) => (params.value != null && params.value !== '-' ? `<strong>${params.name}</strong><br/>Population: ${params.value.toLocaleString()}` : `<strong>${params.name}</strong><br/>No data`),
        },
        visualMap: {
          left: 'right',
          min: 500000,
